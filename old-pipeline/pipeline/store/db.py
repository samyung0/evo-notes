...
literals cast to ::halfvec, so no extra pgvector adapter is required."""
...
def insert_passage(cur, doc_id: str, ws: str, ordn: int, text: str, themes: List[str], emb: str) -> str:
    pid = uid("ps")
    cur.execute(
        "INSERT INTO passages (id, document_id, workspace_id, ord, text, themes, embedding) VALUES (%s,%s,%s,%s,%s,%s,%s::halfvec)",
...
def insert_sentence(cur, passage_id: str, ws: str, ordn: int, text: str, emb: str) -> str:
    sid = uid("sn")
    cur.execute(
        "INSERT INTO sentences (id, passage_id, workspace_id, ord, text, embedding) VALUES (%s,%s,%s,%s,%s,%s::halfvec)",
...
def dense_search(cur, ws: str, query_emb: str, k: int) -> List[Dict[str, Any]]:
    cur.execute(
        """
        SELECT p.id, p.text, d.file_id, f.name, 1 - (p.embedding <=> %s::halfvec) AS score
        FROM passages p
        JOIN documents d ON d.id = p.document_id
        JOIN files f ON f.id = d.file_id
        WHERE p.workspace_id = %s AND p.embedding IS NOT NULL
        ORDER BY p.embedding <=> %s::halfvec ASC
        LIMIT %s
        """,
...
def concept_search(cur, ws: str, query_emb: str, k: int):
    ...
    cur.execute(
        """
        SELECT id, canonical_name, 1 - (embedding <=> %s::halfvec) AS score
...
def bridged_concepts(cur, ws: str, query_emb: str, k: int):
    ...
    cur.execute(
        """
        SELECT sc.concept_id, sum((1 - (s.embedding <=> %s::halfvec)) * sc.weight) AS w
        FROM sentences s
...
def keyword_search(cur, ws: str, query_emb: str, level: str, k: int):
    cur.execute(
        """
        SELECT id, term, 1 - (embedding <=> %s::halfvec) AS score
...
# ----------------------------------------------- LightRAG graph (LLM-extracted)

def document_passages(cur, doc_id: str):
    """(passage_id, text) for a document, in order — used by the lightrag graph
    builder to link LLM entities back to the already-inserted passages."""
    cur.execute("SELECT id, text FROM passages WHERE document_id=%s ORDER BY ord", (doc_id,))
    return [(r[0], r[1]) for r in cur.fetchall()]


def reset_lightrag(cur, ws: str, file_id: str) -> None:
    """Drop the LLM graph rows for a file's passages before a re-ingest.

    lr_passage_entity rows cascade when the passages are deleted by
    reset_document; here we also prune entities/relations that no longer have any
    passage link in this workspace so the graph stays consistent across re-runs."""
    cur.execute(
        """
        DELETE FROM lr_passage_entity pe
        USING passages p, documents d
        WHERE pe.passage_id = p.id AND p.document_id = d.id AND d.file_id = %s
        """,
        (file_id,),
    )
    cur.execute(
        """
        DELETE FROM lr_entities e
        WHERE e.workspace_id = %s
          AND NOT EXISTS (SELECT 1 FROM lr_passage_entity pe WHERE pe.entity_id = e.id)
        """,
        (ws,),
    )


def upsert_entity(cur, ws: str, name: str, etype: str, description: str, emb: str) -> str:
    """Insert or merge an LLM entity by canonical name. On conflict, keep the
    longer description and refresh the embedding/type."""
    cur.execute(
        """
        INSERT INTO lr_entities (id, workspace_id, name, etype, description, embedding)
        VALUES (%s,%s,%s,%s,%s,%s::halfvec)
        ON CONFLICT (workspace_id, name) DO UPDATE SET
            description = CASE WHEN length(EXCLUDED.description) > length(lr_entities.description)
                               THEN EXCLUDED.description ELSE lr_entities.description END,
            etype = EXCLUDED.etype,
            embedding = EXCLUDED.embedding
        RETURNING id
        """,
        (uid("le"), ws, name, etype, description, emb),
    )
    return cur.fetchone()[0]


def link_passage_entity(cur, passage_id: str, entity_id: str, weight: float = 1.0) -> None:
    cur.execute(
        """
        INSERT INTO lr_passage_entity (passage_id, entity_id, weight) VALUES (%s,%s,%s)
        ON CONFLICT (passage_id, entity_id) DO UPDATE SET weight = lr_passage_entity.weight + EXCLUDED.weight
        """,
        (passage_id, entity_id, weight),
    )


def add_lr_relation(cur, ws: str, src: str, dst: str, description: str, keywords: List[str], weight: float, emb: str) -> None:
    if src == dst:
        return
    a, b = (src, dst) if src < dst else (dst, src)  # symmetric edge
    cur.execute(
        """
        INSERT INTO lr_relations (id, workspace_id, src_entity_id, dst_entity_id, description, keywords, weight, embedding)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s::halfvec)
        ON CONFLICT (workspace_id, src_entity_id, dst_entity_id) DO UPDATE SET
            weight = lr_relations.weight + EXCLUDED.weight,
            description = CASE WHEN length(EXCLUDED.description) > length(lr_relations.description)
                              THEN EXCLUDED.description ELSE lr_relations.description END,
            keywords = EXCLUDED.keywords,
            embedding = EXCLUDED.embedding
        """,
        (uid("lr"), ws, a, b, description, keywords, weight, emb),
    )


def entity_search(cur, ws: str, query_emb: str, k: int):
    """LLM entities most similar to the query — the local-mode seed."""
    cur.execute(
        """
        SELECT id, name, 1 - (embedding <=> %s::halfvec) AS score
        FROM lr_entities WHERE workspace_id=%s AND embedding IS NOT NULL
        ORDER BY embedding <=> %s::halfvec LIMIT %s
        """,
        (query_emb, ws, query_emb, k),
    )
    return [(eid, name, float(score)) for eid, name, score in cur.fetchall()]


def relation_search(cur, ws: str, query_emb: str, k: int):
    """LLM relations most similar to the query — the global-mode seed. Returns
    (relation_id, src_entity_id, dst_entity_id, score)."""
    cur.execute(
        """
        SELECT id, src_entity_id, dst_entity_id, 1 - (embedding <=> %s::halfvec) AS score
        FROM lr_relations WHERE workspace_id=%s AND embedding IS NOT NULL
        ORDER BY embedding <=> %s::halfvec LIMIT %s
        """,
        (query_emb, ws, query_emb, k),
    )
    return [(rid, src, dst, float(score)) for rid, src, dst, score in cur.fetchall()]


def relation_neighbors_lr(cur, ws: str, entity_ids: List[str]):
    """One hop over the LLM relation graph from a set of entities. Returns
    (neighbour_entity_id, weight)."""
    if not entity_ids:
        return []
    cur.execute(
        """
        SELECT CASE WHEN src_entity_id = ANY(%s) THEN dst_entity_id ELSE src_entity_id END AS nbr, weight
        FROM lr_relations
        WHERE workspace_id=%s AND (src_entity_id = ANY(%s) OR dst_entity_id = ANY(%s))
        """,
        (entity_ids, ws, entity_ids, entity_ids),
    )
    return [(r[0], float(r[1])) for r in cur.fetchall()]


def passages_for_entities(cur, ws: str, entity_ids: List[str], limit: int):
    if not entity_ids:
        return []
    cur.execute(
        """
        SELECT p.id, p.text, d.file_id, f.name, sum(pe.weight) AS w
        FROM passages p
        JOIN lr_passage_entity pe ON pe.passage_id = p.id
        JOIN documents d ON d.id = p.document_id
        JOIN files f ON f.id = d.file_id
        WHERE p.workspace_id=%s AND pe.entity_id = ANY(%s)
        GROUP BY p.id, p.text, d.file_id, f.name
        ORDER BY w DESC LIMIT %s
        """,
        (ws, entity_ids, limit),
...
def purge_workspace(cur, ws_id: str) -> None:
    ...
    cur.execute("DELETE FROM lr_entities WHERE workspace_id=%s", (ws_id,))  # cascades lr_relations + lr_passage_entity
