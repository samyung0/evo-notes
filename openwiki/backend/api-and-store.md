...
**Huma JSON CRUD** — 60+ endpoints auto-documented in OpenAPI, organized by tag: Workspaces, Content, Source Upload Policy, Materials, Collaboration, Membership, Chat, Quizzes, Flashcards, Sharing, Schedule, Tags, Thinking, Account, Billing & Integrations, Explore.
...
### Source upload policy and chapter intent

`server/internal/sourceupload` is the single backend source of truth for the
complete extension/kind allowlist copied from `AddSourceDialog.tsx`, parser
allowlists, and size limits. `GET /api/source-upload-policy` is registered
through Huma so the frontend can consume the generated contract rather than
duplicating those lists. Binary upload and presigned B2 routes remain raw chi
routes because Huma does not model their storage-provider handoff.

Uploads may carry either `chapterId` or `chapterName`. A new chapter name is
stored on a direct-upload session and resolved only during finalization; the
multipart path resolves it while inserting the file/job. Both paths
find-or-create case-insensitively inside the file-creation transaction, so an
abandoned direct upload does not leave an empty chapter. Expired pending
sessions are cleaned opportunistically (including their blob objects), and
their unfinalized chapter intent is discarded.
...
- **`0003_upload_chapter_names.sql`** — Persists pending chapter intent for direct source uploads until atomic finalization.
...
| `jobs.go`          | `CreateSourceWithJob` (resolves chapter intent and inserts file + job in one transaction), Postgres-backed queue claimed via `SKIP LOCKED` |
| `uploads.go`       | UploadSession lifecycle (presigned B2), pending chapter intent, idempotent finalization                                                    |
