...
const setEq = (a: number[], b: number[]) => a.length === b.length && [...a].sort().every((v, i) => v === [...b].sort()[i]);
...
export function gradeQuestion(q: Question, answer: Answer | undefined): boolean {
  ...
  switch (q.type) {
    case 'mcq':
    case 'multi':
      return Array.isArray(answer) && typeof answer[0] !== 'string' && setEq(answer as number[], q.correct);
    case 'boolean':
