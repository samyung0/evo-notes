...
export function TimeGrid({
  ...
  function colorFor(ev: CalendarEvent) {
    ...
    return first ? colorPair(first.color) : { bg: 'var(--surface-inset-bg)', fg: 'var(--text-secondary)', fgMuted: 'var(--text-muted)', solid: 'var(--text-muted)' };
  }
    ...
    <div className="flex flex-col">
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.toISOString()} className="flex-1 py-2 text-center">
              <div className="text-[0.7rem] font-semibold text-fg-muted">{WEEKDAY_SHORT[(d.getDay() + 6) % 7]}</div>
              <div className={cn('mx-auto mt-0.5 flex h-7 w-7 items-center justify-center rounded-pill text-sm', isToday ? 'bg-action font-bold text-action-fg' : 'text-fg')}>{d.getDate()}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="relative flex" style={{ height: HOUR_H * 24 }}>
        ...
        <div className="w-14 shrink-0">
          {Array.from({ length: 24 }, (_, h) => (
            <div key={h} className="relative" style={{ height: HOUR_H }}>
              <span className="absolute -top-2 right-2 text-[0.66rem] text-fg-muted">{h === 0 ? '' : `${h}:00`}</span>
            </div>
          ))}
        </div>
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.toISOString()} className={cn('relative flex-1 border-l border-divider', isToday && 'bg-inset/40')}>
              {/* gridlines */}
              {Array.from({ length: 24 }, (_, h) => (
              ...
              {dayEvents.map((ev) => {
                ...
                return (
                  <button
                    ...
                    className={cn('absolute left-1 right-1 overflow-hidden rounded-row px-2 py-1 text-left', selectedId === ev.id && 'ring-2 ring-fg')}
                    style={{ top, height, background: c.bg, color: c.fg }}
                  >
                    <span className="block truncate text-[0.72rem] font-bold">{ev.title}</span>
                    <span className="block truncate text-[0.62rem] opacity-80">{fmtTime(ev.start)}</span>
                  </button>
                );
              })}
              {/* current time line */}
              {isToday && (
                <div className="pointer-events-none absolute left-0 right-0 z-10" style={{ top: nowTop }}>
                  <div className="relative border-t-2 border-solid-error">
                    <span className="absolute -left-1 -top-1 h-2 w-2 rounded-full bg-solid-error" />
                  </div>
                </div>
              )}
            </div>
