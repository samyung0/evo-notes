...
export function StudyBlockTimeLimit({ minutes }: { minutes?: number }) {
  if (minutes == null) return null;
  return (
    <div contentEditable={false} className="t-meta text-fg-muted">
  ...
  return (
    <>
      <div
        ...
        className="t-meta col-span-2 flex items-center justify-between gap-3 text-fg-muted"
      >
        <span>{questionNumber == null ? 'Question' : `Question ${questionNumber}`}</span>
        <span>{questionTypeLabel(questionType)}</span>
      </div>
