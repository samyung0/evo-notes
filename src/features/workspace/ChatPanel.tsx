...
export function ChatPanel({ workspaceId }: { workspaceId: string }) {
  ...
  function send() {
    ...
    setText('');
    chat.mutate(trimmed, { onSuccess: (reply) => setMessages((prev) => [...prev, reply]) });
  }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-auto p-4">
        ...
        {messages.map((msg) =>
          msg.role === 'user' ? (
            <div key={msg.id} className="ml-auto max-w-[85%] rounded-[14px] rounded-tr-[4px] bg-action px-3.5 py-2.5 text-sm text-action-fg">
              {msg.text}
            </div>
          ) : (
            <div key={msg.id} className="mr-auto max-w-[90%] rounded-[14px] rounded-tl-[4px] border border-line bg-surface px-3.5 py-2.5 text-sm text-fg">
              <p className="m-0">{msg.text}</p>
              {msg.citations && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {msg.citations.map((c) => (
                    <span key={c.fileId} className="inline-flex items-center gap-1 rounded-pill bg-tint-info px-2 py-0.5 text-[11px] font-medium text-tint-info-fg">
        ...
        {chat.isPending && <div className="mr-auto"><Spinner size={16} /></div>}
      </div>
