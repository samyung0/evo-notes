...
export default function Flashcards() {
  ...
  return (
    <Panel>
      <PageHeader title={m.nav_flashcards()} actions={<IconButton icon="plus" variant="dark" label="New deck" />} />
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {data?.map((d) => {
              ...
              return (
                <Link key={d.id} to="/flashcards/$deckId" params={{ deckId: d.id }} preload="intent">
                  <Card interactive padding={18} radius="card-lg" className="h-full">
                    ...
                    <Text variant="card-title" className="mt-3 truncate">{d.name}</Text>
                    <Text variant="meta" tone="muted" className="mt-0.5">{d.workspaceName}</Text>
                    <div className="mt-4">
