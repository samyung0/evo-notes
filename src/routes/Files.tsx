      ...
      <div className="min-h-0 flex-1 overflow-auto px-6 py-5">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {data?.map((f) => (
              <Card key={f.id} interactive padding={16} onClick={() => setOpen(f)} className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-card bg-inset text-fg-soft"><Icon name="files" size={18} /></span>
                <div className="min-w-0 flex-1">
                  <Text variant="subtitle" className="truncate">{f.name}</Text>
                  <Text variant="meta" tone="muted">{(f.sizeKb / 1024).toFixed(1)} MB</Text>
                </div>
                <Badge tone="neutral" size="sm">{f.kind}</Badge>
              </Card>
            ))}
          </div>
        )}
