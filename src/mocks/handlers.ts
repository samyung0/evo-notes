...
export const handlers = [
  ...
  http.get('/api/search', async ({ request }) => {
    ...
    for (const w of db.workspaces)
      if (w.name.toLowerCase().includes(q) || w.tags.some((t) => t.toLowerCase().includes(q)))
        results.push({ id: w.id, kind: 'workspace', title: w.name, subtitle: w.tags.join(' · '), href: `/workspaces/${w.id}` });
    for (const f of db.files)
      if (f.name.toLowerCase().includes(q))
        results.push({ id: f.id, kind: 'file', title: f.name, subtitle: db.workspaces.find((w) => w.id === f.workspaceId)?.name, href: `/workspaces/${f.workspaceId}?file=${f.id}` });
    for (const e of db.events)
      if (e.title.toLowerCase().includes(q))
        results.push({ id: e.id, kind: 'event', title: e.title, subtitle: e.location, href: '/schedule' });
    for (const d of db.decks)
      if (d.name.toLowerCase().includes(q))
        results.push({ id: d.id, kind: 'flashcards', title: d.name, subtitle: d.workspaceName, href: `/flashcards/${d.id}` });
    for (const c of db.canvases)
      if (c.name.toLowerCase().includes(q))
        results.push({ id: c.id, kind: 'thinking', title: c.name, href: `/thinking/${c.id}` });
  ...
  http.get('/api/workspaces', async ({ request }) => {
    ...
    if (q) list = list.filter((w) => w.name.toLowerCase().includes(q) || w.tags.some((t) => t.toLowerCase().includes(q)));
  ...
  http.get('/api/workspaces/:id/stats', async ({ params }) => {
    ...
    const att = db.attempts.filter((a) => db.quizzes.find((q) => q.id === a.quizId)?.workspaceId === ws.id);
    const avg = att.length ? Math.round(att.reduce((s, a) => s + a.pct, 0) / att.length) : 0;
    return HttpResponse.json({ chapters: ws.chapterCount, files: ws.fileCount, quizzes: quizCount, attempts: att.length, avgScore: avg });
  }),
  ...
  http.get('/api/workspaces/:id/chapters', async ({ params }) => {
    await latency();
    return HttpResponse.json(db.chapters.filter((c) => c.workspaceId === params.id).sort((a, b) => a.order - b.order));
  }),
  http.post('/api/workspaces/:id/chapters', async ({ params, request }) => {
    ...
    const ch: Chapter = { id: uid('ch'), workspaceId: String(params.id), name: body.name, order, fileIds: [] };
    db.chapters.push(ch);
    const ws = db.workspaces.find((w) => w.id === params.id);
  ...
  http.post('/api/workspaces/:id/sources', async ({ params, request }) => {
    await delay(500);
    const body = (await request.json()) as { name: string; kind: SourceKindFix; chapterId?: string | null };
  ...
  http.post('/api/workspaces/:id/chat', async ({ request }) => {
    ...
    return HttpResponse.json({
      ...
      citations: sources.map((f) => ({ fileId: f.id, fileName: f.name, snippet: 'Relevant passage from your source…' })),
    });
  }),
  http.post('/api/workspaces/:id/generate', async ({ params, request }) => {
    ...
    if (opts.kind === 'summary') {
      return HttpResponse.json({ kind: 'summary', title: `${wsName} summary`, body: '• The cell is the basic unit of life.\n• Mitochondria produce ATP.\n• Membranes control transport via diffusion and osmosis.' });
    }
    if (opts.kind === 'flashcards') {
      const seed: Flashcard[] = Array.from({ length: opts.count }, (_, i) => ({ id: uid('c'), deckId: 'generated', front: `Term ${i + 1}`, back: `Definition for term ${i + 1}.`, known: false }));
    ...
    const qs: Question[] = Array.from({ length: opts.count }, (_, i) => {
      ...
      const base = { id: uid('q'), difficulty, prompt: `Generated ${type} question ${i + 1}?` };
      switch (type) {
        case 'boolean':
        ...
        case 'ordering':
          return { ...base, type: 'ordering', items: ['First', 'Second', 'Third'] } as Question;
        case 'matching':
          return { ...base, type: 'matching', pairs: [{ left: 'A', right: '1' }, { left: 'B', right: '2' }] } as Question;
        case 'multi':
          return { ...base, type: 'multi', options: ['A', 'B', 'C', 'D'], correct: [0, 2] } as Question;
        default:
          return { ...base, type: 'mcq', options: ['A', 'B', 'C', 'D'], correct: [0] } as Question;
      }
    });
    const quiz: Quiz = { id: uid('qz'), name: `${wsName} quiz`, workspaceId: String(params.id), workspaceName: wsName, chapters: opts.chapters, questions: qs, createdAt: new Date().toISOString(), privacy: 'private', timeLimitMin: opts.timeLimitMin };
  ...
  http.get('/api/attempts', async () => {
    await latency();
    return HttpResponse.json([...db.attempts].sort((a, b) => +new Date(b.takenAt) - +new Date(a.takenAt)));
  }),
  ...
  http.post('/api/thinking', async ({ request }) => {
    const body = (await request.json()) as { name: string };
    const c = { id: uid('cv'), name: body.name, updatedAt: new Date().toISOString() };
