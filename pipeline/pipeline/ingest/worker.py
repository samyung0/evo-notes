...
def process_job(conn, embedder, job: dict) -> None:
    ...
    try:
        indexer.index_document(corpus, embedder, ws, file_id, parser.name, passages, doc.pages)
        with conn.cursor() as cur:
