...
DELETE FROM materials WHERE user_id IN ('u_owner', 'u_editor', 'u_viewer', 'u_other');
...
INSERT INTO workspaces (id, user_id, name, color, privacy, created_at, last_accessed_at) VALUES
  ('ws_e2e_private', 'u_owner', 'E2E Private Workspace', 'green',  'private', now(), now()),
  ('ws_e2e_link',    'u_owner', 'E2E Link Workspace',    'purple', 'link',    now(), now()),
  ('ws_e2e_public',  'u_owner', 'E2E Public Workspace',  'blue',   'public',  now(), now()),
  ('ws_e2e_mutate',  'u_owner', 'E2E Mutate Workspace',  'amber',  'private', now(), now())
ON CONFLICT (id) DO UPDATE SET
